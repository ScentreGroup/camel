...
Google security credentials can be set explicitly via one of the two options when using the `GooglePubsubConnectionFactory`:
...
For advanced configuration options a `ChannelProvider` can be supplied allowing full control over credentials.

### URI Format

[source,java]
-------------------------------------
google-pubsub:projectId:destinationName[?options]
-------------------------------------

Where `projectId` is the google cloud project id and `destinationName` is the
pubsub topic or subscription name.
...
| **connectionFactory** (common) | Sets the connection factory to use: provides the ability to explicitly manage connection credentials: - the path to the key file - the Service Account Key / Email pair. Will take precedence over `channelProvider`. |  | GooglePubsubConnection Factory
