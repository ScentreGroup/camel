...
Security credentials is controlled through a SecurityPRovidercan be set explicitly via one of the two options:
...
| **credetialsProvider** (common) | Sets the credentials provider to use: provides the ability to explicitly set the channel credentials provider |  | com.google.api.gax.core.CredentialsProvider
| **executorProvider** (common) | Sets the executor service provider to use: provides the ability to explicitly set the executor provider |  | com.google.api.gax.grpc.ExecutorProvider
